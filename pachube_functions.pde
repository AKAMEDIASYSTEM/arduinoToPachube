...
void pachube_in_out(){
  ...
  if (ready_to_update){
    ...
    if (localClient.connect()) {
      ...
      float tempSample = tmp36(tempPin);

      sprintf(pachube_data,"%d,%d",(int)tempSample,analogRead(photoPin));
      content_length = strlen(pachube_data);


      Serial.println("now updating");
...
float tmp36(int whichPin){
  int tempReading = analogRead(whichPin);  
